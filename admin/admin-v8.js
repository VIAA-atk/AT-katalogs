...
let observedEditorSnapshot = "";
let observedEditorSince = 0;
...
function editorSnapshot() {
  if (ui["editor-fields"].disabled || (!selectedId && !newRecord)) return "";
  const fields = [...ui["record-form"].querySelectorAll("input, select, textarea")]
    .filter((element) => element.type !== "file")
    .map((element) => [element.name, element.value, element.type === "checkbox" ? element.checked : null]);
  const file = ui["field-image-file"].files[0];
  return JSON.stringify({ fields, file: file ? [file.name, file.size, file.lastModified] : null });
}

function rememberEditorSnapshot() {
  lastEditorSnapshot = editorSnapshot();
  observedEditorSnapshot = lastEditorSnapshot;
  observedEditorSince = Date.now();
}
...
async function connect(event) {
  ...
  try {
    ...
    ui["connection-info"].textContent = `Savienots kā ${profile.login}; ${resources.length} ieraksti; zars ${repository.branch}; paneļa versija 8.`;
...
window.setInterval(() => {
  if (!token || busy || savingDraft) return;
  const current = editorSnapshot();
  if (!current || current === lastEditorSnapshot) {
    observedEditorSnapshot = current;
    observedEditorSince = Date.now();
    return;
  }
  if (current !== observedEditorSnapshot) {
    observedEditorSnapshot = current;
    observedEditorSince = Date.now();
    return;
  }
  if (Date.now() - observedEditorSince < 1200) return;
  lastEditorSnapshot = current;
  saveDraft();
}, 400);
